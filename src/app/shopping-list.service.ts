...
export class ShoppingListService {
  ...
  getShoppingList(sortBy = 'metadata.createdAt', orderBy = -1, pageIndex, pageSize): Observable<ShoppingListResponse> {
    const query = `?sortby=${sortBy}&orderby=${orderBy}&pageno=${pageIndex ? pageIndex : 0}&pagesize=${pageSize ? pageSize : 10}`;
    return this.http.get<ShoppingListResponse>(`/api/list/shoppinglist${query}`);
  }
